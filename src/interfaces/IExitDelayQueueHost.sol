...
///         `keccak256("sovryn.exitDelayQueue") - 1`, set by `setExitDelayQueue`
///         (gated by the SAME host admin as `setExitFeeController`, Owner/Timelock,
///         and rotatable) and read back via `exitDelayQueue()`.
///
/// @dev    The host CONTRACTS live in the product repos (Sovryn-smart-contracts-colfee
///         / zero-contracts-colfee) on the 0.5.x / 0.6.x pragmas; this 0.8.20 stub
///         exists ONLY so the colfee deploy/wire script (05) can call the pointer
...
interface IExitDelayQueueHost {
    ...
    ///         Owner/Timelock-gated on the host; a no-op in colfee (hosts are external).
